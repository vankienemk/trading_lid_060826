"""Causal sweep-level feature registry (subset for signal engine).
The signal engine uses a hard-coded feature list matching the frozen model."""
from __future__ import annotations

SIGNAL_FEATURE_NAMES = [
    "atr_percentile",
    "bars_since_last_touch",
    "body_ratio",
    "close_location",
    "confirmation_body_ratio",
    "confirmation_delay_bars",
    "confirmation_range_atr",
    "confirmation_volume_zscore",
    "day_of_week",
    "distance_to_previous_day_high_atr",
    "distance_to_previous_day_low_atr",
    "equal_level_dispersion_atr",
    "h1_trend",
    "hour_utc",
    "level_age_bars",
    "level_already_partially_swept",
    "level_is_h1_swing",
    "level_is_previous_day_high_low",
    "level_touch_count",
    "lower_wick_ratio",
    "penetration_atr",
    "range_atr",
    "reclaim_atr",
    "session_asia",
    "session_london",
    "session_new_york",
    "sweep_volume_zscore",
    "upper_wick_ratio",
    "volume_zscore",
    "wick_ratio",
]


def registered_feature_names() -> list[str]:
    return list(SIGNAL_FEATURE_NAMES)


__all__ = ["SIGNAL_FEATURE_NAMES", "registered_feature_names"]